...
| 2 | Global — `src/index.css` (83 usos de `fadeUp`) | `prefers-reduced-motion` sólo se respeta en el bot (`Acompanante.jsx:79`, `Criatura.jsx:83`). Las 83 entradas de página se mueven igual para todo el mundo | Accesibilidad — es un defecto, no un adorno | Todos | En `index.css`: `@media (prefers-reduced-motion: reduce) { .animate-\[fadeUp*\], .gk-anim-* { animation-duration: 0.01ms !important; } }`. **Atenuar, no eliminar**: quitar la opacidad de golpe hace que el contenido parpadee |
