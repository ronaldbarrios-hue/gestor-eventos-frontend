  ...
  return (
    <div className="max-w-[1500px] mx-auto flex gap-4 h-[calc(100vh-7rem)]">
      <aside className="hidden md:flex w-[220px] flex-shrink-0 flex-col rounded-3xl border border-border-2 bg-surface/70 overflow-hidden">
        <div className="p-3 border-b border-border">
          <button onClick={crearConv}
            className="w-full rounded-xl bg-gradient-primary py-2.5 text-sm font-semibold text-white hover:opacity-90 active:scale-95 transition flex items-center justify-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          ...
          {convs.map(c => (
            <div key={c.id}
              className={`group flex items-center gap-1 rounded-xl px-2.5 py-2 cursor-pointer transition ${c.id === activa ? 'bg-surface-2 border border-primary/40' : 'hover:bg-surface-2 border border-transparent'}`}
              onClick={() => { setActiva(c.id); setForm(null); }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="text-text-3 flex-shrink-0">
              ...
              <button onClick={(e) => { e.stopPropagation(); borrarConv(c.id); }}
                className="opacity-0 group-hover:opacity-100 text-text-3 hover:text-danger transition" aria-label="Borrar">
      ...
      <div className="flex-1 flex flex-col rounded-3xl border border-border-2 bg-surface/70 overflow-hidden">
        <div className="relative flex items-center gap-4 px-5 py-4 border-b border-border bg-gradient-to-br from-surface-2 to-surface overflow-hidden">
          <div className="absolute inset-0 bg-grid-pattern opacity-25" />
          <Criatura mood={moodActual} size={92} />
          <div className="relative flex-1 min-w-0">
            <h2 className="text-xl font-display font-bold text-text-1">Gestbot</h2>
            <p className={`text-sm font-medium ${cargando ? 'text-primary-light' : mood === 'error' ? 'text-danger' : mood === 'happy' ? 'text-success' : 'text-text-2'}`}>
              {estadoTxt}
            </p>
            {cargando && (
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 sm:px-6 py-5 space-y-3">
          {mensajes.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[82%] rounded-2xl px-4 py-3 text-[15px] leading-relaxed whitespace-pre-wrap ${m.role === 'user' ? 'bg-primary text-white rounded-br-sm' : 'bg-surface-2 text-text-1 border border-border rounded-bl-sm'}`}>
                {m.content}
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {SUGERENCIAS.map((s, i) => (
                <button key={i} onClick={() => enviar(s)}
                  className="text-sm text-left px-3.5 py-2.5 rounded-xl bg-surface-2 border border-border text-text-2 hover:text-text-1 hover:border-primary/50 transition">{s}</button>
              ))}
            </div>
          )}
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); enviar(); }}
          ...
          <input ref={fileRef} type="file" multiple accept="image/*,application/pdf" onChange={onFiles} className="hidden" />
          <button type="button" onClick={() => fileRef.current?.click()} disabled={cargando} aria-label="Adjuntar PDF o foto"
            className="shrink-0 rounded-xl border border-border p-3 text-text-2 hover:text-text-1 hover:border-primary/50 disabled:opacity-40 transition">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M21.44 11.05l-9.19 9.19a5 5 0 01-7.07-7.07l9.19-9.19a3.5 3.5 0 014.95 4.95l-9.2 9.19a2 2 0 01-2.83-2.83l8.49-8.48" strokeLinecap="round" strokeLinejoin="round" /></svg>
          </button>
          <textarea rows={1} value={input} onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); enviar(); } }}
            placeholder="Escríbele a Gestbot…  (o adjunta un PDF/foto)"
            disabled={cargando}
            className="flex-1 resize-none max-h-32 rounded-xl bg-surface border border-border px-3.5 py-3 text-[15px] text-text-1 placeholder:text-text-3 focus:outline-none focus:border-primary/60 disabled:opacity-60" />
          <button type="submit" disabled={cargando || (!input.trim() && adjuntos.length === 0)} aria-label="Enviar"
            className="shrink-0 rounded-xl bg-gradient-primary p-3 text-white disabled:opacity-40 hover:opacity-90 active:scale-95 transition">
      ...
      <aside className="hidden xl:flex w-[380px] flex-shrink-0 flex-col rounded-3xl border border-border-2 bg-surface/70 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {evPrev ? <EventoPreview ev={evPrev} /> : (
            <div className="h-full flex flex-col items-center justify-center text-center text-text-3 gap-3">
              <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" className="text-text-3/60">
                <rect x="3" y="4" width="18" height="17" rx="2" />
                <path d="M3 9h18M8 2v4M16 2v4M8 14h5" strokeLinecap="round" />
              </svg>
              <p className="text-sm max-w-[220px]">Pídele a Gestbot crear un evento (o súbele un PDF/foto) y aquí verás la vista previa en grande.</p>
            </div>
          )}
        </div>
      </aside>
...
function ResultadoView({ resultado }) {
      ...
      <div className="rounded-xl border border-border bg-surface/60 divide-y divide-border">
        {items.slice(0, 8).map((it, i) => (
          <div key={i} className="px-3 py-2 text-[13px]">
            {typeof it === 'object' && it ? (
              <>
                <p className="text-text-1 font-medium truncate">{it.titulo || it.nombre || it.evento || it.codigo || it.recompensa || `#${i + 1}`}</p>
                <p className="text-text-3 truncate">
                  {[it.estado, it.fecha_inicio || it.fecha || it.vence, it.email, it.puntos != null ? it.puntos + ' pts' : null, it.precio != null ? '$' + it.precio : null].filter(Boolean).join(' · ')}
                </p>
              </>
  ...
  const pares = Object.entries(resultado).filter(([k, v]) => k !== 'ok' && (typeof v === 'string' || typeof v === 'number' || typeof v === 'boolean'));
...
function EventoPreview({ ev }) {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="h-44 rounded-2xl bg-gradient-to-br from-primary/30 to-accent/20 border border-border flex items-center justify-center">
...
function FormAgente({ form, onSubmit, onCancel }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    const faltan = (form.campos || []).filter(c => c.requerido !== false && (vals[c.clave] == null || String(vals[c.clave]).trim() === ''));
        ...
        <div key={c.clave || i} className="space-y-1">
          ...
          {c.tipo === 'textarea' ? (
            <textarea rows={2} placeholder={c.placeholder || ''} value={vals[c.clave] || ''}
              ...
              className="w-full resize-none rounded-lg bg-surface border border-border px-3 py-2 text-sm text-text-1 placeholder:text-text-3 focus:outline-none focus:border-primary/60" />
          ) : c.tipo === 'opcion' ? (
            <select value={vals[c.clave] || ''} onChange={(e) => set(c.clave, e.target.value)}
              className="w-full rounded-lg bg-surface border border-border px-3 py-2 text-sm text-text-1 focus:outline-none focus:border-primary/60">
            ...
            <input type={TIPO_INPUT[c.tipo] || 'text'} placeholder={c.placeholder || ''} value={vals[c.clave] || ''}
              onChange={(e) => set(c.clave, e.target.value)}
              className="w-full rounded-lg bg-surface border border-border px-3 py-2 text-sm text-text-1 placeholder:text-text-3 focus:outline-none focus:border-primary/60" />
          )}
        </div>
      ))}
      {vals.__err && <p className="text-sm text-danger">{vals.__err}</p>}
      <div className="flex gap-2 pt-1">
        <button type="submit" className="flex-1 rounded-lg bg-gradient-primary py-2.5 text-sm font-semibold text-white hover:opacity-90 active:scale-95 transition">Enviar a Gestbot</button>
        <button type="button" onClick={onCancel} className="rounded-lg border border-border px-4 py-2.5 text-sm text-text-2 hover:text-text-1 hover:border-border-2 transition">Cancelar</button>
      </div>
    </form>
  );
}
