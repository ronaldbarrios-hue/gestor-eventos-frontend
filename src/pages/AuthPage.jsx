...
export default function CompletarPerfilPage() {
  const { usuario, logout, checkInvitacionPendiente, consumirInvitacionRedirect } = useAuth();
  ...
  /* Si este usuario tiene una invitación pendiente a un equipo, ocultamos
     las preguntas de "organizador" (empresa, tamaño de eventos, etc). */
  const [invitacion, setInvitacion] = useState(null);
  const [checkingInvite, setCheckingInvite] = useState(true);

  useEffect(() => {
    if (!usuario?.email) { setCheckingInvite(false); return; }
    checkInvitacionPendiente(usuario.email)
      .then(info => setInvitacion(info?.invitado ? info : null))
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const eventoId = consumirInvitacionRedirect() || invitacion?.eventoId;
      navigate(eventoId ? `/eventos/${eventoId}` : '/dashboard', { replace: true });
    } catch (e) {
      setErr(e.message);
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-bg text-text-1 overflow-hidden px-4 py-12 animate-[fadeIn_0.4s_ease_both]">
      ...
      <div className="relative w-full max-w-xl">
        ...
        {!checkingInvite && invitacion?.invitado && (
          <div {...stagger(3)} className={`${stagger(3).className} px-4 py-3 rounded-2xl bg-primary/10 border border-primary/20 text-sm text-text-2 leading-relaxed mb-6`}>
            ...
            {invitacion.eventoTitulo ? <> a <strong className="text-text-1">{invitacion.eventoTitulo}</strong></> : ''}. Al terminar, entrarás directo a ese evento.
          </div>
        )}
