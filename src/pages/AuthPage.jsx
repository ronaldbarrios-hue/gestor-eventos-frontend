...
/*
  FIX: stagger() antes retornaba un objeto con className + style mezclados para
  hacer spread en JSX. Eso funciona en desktop pero en algunos entornos móviles
  React no aplica bien el `style` cuando viene del spread junto con otros props.
  Ahora devolvemos solo className con la variable CSS inyectada directamente
  via style inline separado, y exportamos un helper separado para el estilo.
*/
...
export default function AuthPage() {
  ...
  /*
    FIX animaciones de transición:
    Las clases animate-auth-* son Tailwind arbitrarias que requieren definición
    en tailwind.config. Si no están definidas, en móvil simplemente no se aplica
    nada y el contenido salta bruscamente. Usamos clases inline con keyframes
    estándar de Tailwind que sí existen garantizados.
    
    Simplificamos a fadeIn/fadeOut para garantizar compatibilidad móvil.
    Si quieres mantener el slide lateral, asegúrate de tenerlas en el config.
  */
  ...
  return (
    /*
      FIX layout móvil:
      1. Cambié overflow-hidden → overflow-x-hidden para no cortar el scroll vertical
      2. Añadí pt-16 para que el botón "Volver" fixed no tape el contenido
      3. min-h-screen con flex y pb-8 para respiración inferior
    */
    <div className="relative min-h-screen flex items-start lg:items-center justify-center bg-bg text-text-1 overflow-x-hidden px-4 sm:px-8 pt-16 pb-8 lg:py-12 animate-[fadeIn_0.4s_ease_both]">
      ...
      {/* Back button — FIX: z-40 para estar sobre el blur, tamaño touch-friendly (min 44px) */}
      <Link
        to="/"
...
function BackgroundGlows({ isLogin }) {
  return (
    /* FIX: -z-10 en lugar de -z-0 para asegurarnos que queda detrás de todo */
...
function LoginForm() {
  ...
  return (
    <div>
      ...
      {/* FIX: min-h-[44px] para área táctil mínima recomendada */}
      <button
        type="button"
      ...
      <form onSubmit={onSubmit} className={`${staggerClass} space-y-5`} style={staggerStyle(4)}>
        ...
        <div className="field">
          ...
          <input
            ...
            /* FIX: font-size mínimo 16px en móvil para evitar zoom automático de iOS */
        ...
        <div className="field">
          ...
          <div className="relative">
            <input
              ...
              style={{ fontSize: '16px' }}
            />
            {/* FIX: área táctil del botón ojo mínima 44x44 */}
            <button
...
function RegisterForm() {
  ...
  const submitPaso1 = e => {
    ...
    setStep(2);
    /* FIX: scroll al top al cambiar de paso en móvil */
    ...
    <div>
        ...
        <div className={`${staggerClass} flex items-center gap-2 mb-7`} style={staggerStyle(0)}>
          {[1, 2].map(n => (
            <div key={n} className="flex items-center gap-2 flex-1 min-w-0">
              ...
              {/* FIX: truncate en el label para que no desborde en pantallas pequeñas */}
      ...
      {step === 'sent' && (
        <div className="text-center py-6">
          ...
          {/* FIX: break-all para emails largos que desbordan en móvil */}
      ...
      {step === 1 && (
        <div key="step1" className="animate-[fadeUp_0.4s_cubic-bezier(0.16,1,0.3,1)_both]">
          ...
          <form onSubmit={submitPaso1} className={`${staggerClass} space-y-4`} style={staggerStyle(5)}>
            ...
            {/* FIX: en móvil el grid de 2 cols queda muy apretado — usamos 1 col en móvil */}
            ...
            <div className="field">
              ...
              {/*
                FIX: El grid-cols-[110px_1fr] en pantallas de 320px dejaba el select
                y el input muy apretados. Ahora el selector de código es más estrecho
                y usamos min-w-0 en el input para que no desborde.
              */}
      ...
      {step === 2 && (
        <div key="step2" className="animate-[fadeUp_0.4s_cubic-bezier(0.16,1,0.3,1)_both]">
          ...
          <form onSubmit={submitFinal} className="space-y-4">
            ...
            {/* FIX: área táctil del checkbox ampliada con padding */}
