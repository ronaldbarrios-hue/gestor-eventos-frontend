...
function StandsEditor({ evento, soyOwner }) {
  ...
  const abrirNuevo   = () => { setEditando('nuevo'); setForm({ nombre: '', stand: '', zona_id: '', descripcion: '', logo_url: '', sitio_web: '' }); };
  const abrirEdicion = (s) => { setEditando(s.id); setForm({ nombre: s.nombre || '', stand: s.stand || '', zona_id: s.zona_id || '', descripcion: s.descripcion || '', logo_url: s.logo_url || '', sitio_web: s.sitio_web || '' }); };
